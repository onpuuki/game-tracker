...
- 【手抜き要約の完全禁止】概要（summary）において、「詳細は公式サイトをご参照ください」「ゲーム内で確認してください」といったメタな定型文や逃げの表現を出力することを絶対禁じます。テキストが少ない場合でも、ページの見出し等から「目的」「報酬」「条件」を最大限推測し、具体的な文章を必ず構成してください。
- 【差分出力の絶対ルール】コスト削減のため、入力された【既存のイベント一覧】と内容が完全に同一であり、更新の必要がないイベントは、絶対に events 配列に含めないでください。全てのイベントに変更がない（差分ゼロの）場合は、必ず空の配列 {"events": []} を出力して推論を終了してください。
