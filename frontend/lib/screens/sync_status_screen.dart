              ...
              children: [
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      const text = r'''以下は、私のアプリからエクスポートしたゲームイベントとユーザーフィードバックの一覧データです。
このデータと、あなたの持つ最新のウェブ検索能力や知識を用いて、以下の観点でデータの監査・分析を行ってください。

【監査・分析の観点】
1. 情報の誤り: 期間（開始日・終了日）やタイトルなど、誤ったイベント情報が設定されていないか。
2. 存在確認: 記載されているイベントが公式で本当に存在・開催されているか。
3. 重複確認: 実質的に同じイベントが重複して登録されていないか。
4. 抜け漏れ確認: 現在開催中、または近日開催予定であるにもかかわらず、一覧に掲載されていない重要なイベントがないか。
5. フィードバックの妥当性: ユーザーからのフィードバック内容が事実に基づいているか、対応が必要な正当な指摘か。

上記を分析した結果、データの追加・修正・削除が必要な箇所が見つかった場合、私のアプリに組み込まれているデータベース修正用APIに渡すための「修正指示プロンプト」を作成してください。
以下のフォーマットに厳密に従い、コードブロックで出力してください。複数ある場合は、このブロックを繰り返してください。
